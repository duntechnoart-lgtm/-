...
class PretrainZinkDataset(Dataset):
    def __init__(self, node_features, adj_features, mol_sizes):        
        ...
        # --- MODIFICATION START ---
        # 扩展的原子列表 (12个原子 + 1个虚拟原子 0)
        # C(6) N(7) O(8) F(9) P(15) S(16) Cl(17) Br(35) I(53) B(5) Si(14) Se(34) 虚拟(0)
        self.atom_list = [6, 7, 8, 9, 15, 16, 17, 35, 53, 5, 14, 34, 0]
        self.max_size = self.node_features.shape[1]
        self.node_dim = len(self.atom_list) # 13
        # --- MODIFICATION END ---
    ...
    def __getitem__(self, idx):
        ...
        # --- MODIFICATION START: Node Feature Encoding ---
        # node_feature shape is (N, 13)
        node_feature = np.zeros((self.max_size, self.node_dim), dtype=np.float32) 
        for i in range(self.max_size):
            # 找到对应原子的索引 (0-12)
            index = self.atom_list.index(node_feature_copy[i])
            node_feature[i, index] = 1.
            
        # 移除最后一个维度 (虚拟原子/填充维度)，得到 (N, 12)
        node_feature = node_feature[:, :-1] #(N, 12) 
        # --- MODIFICATION END ---
        
        # Adj feature shape is (4, N, N)
...
class ConstrainOptim_Zink800(Dataset):
    def __init__(self, node_features, adj_features, mol_sizes, raw_smiles, plogps):        
        ...
        # --- MODIFICATION START ---
        # C N O F P S Cl Br I B Si Se 虚拟
        self.atom_list = [6, 7, 8, 9, 15, 16, 17, 35, 53, 5, 14, 34, 0]
        self.max_size = self.node_features.shape[1]
        self.node_dim = len(self.atom_list) # 13
        # --- MODIFICATION END ---
